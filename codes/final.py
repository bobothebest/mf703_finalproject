...
###functions of nnf partial
###
def data_process(df):
    '''
    input:daily return (dataframe)
    output:cumulative return (tensor)
    '''
    df = df + 1
    df = df.cumprod()
    df = df - 1
...
def index_finder():
    ...
    '''
    # stock_full_file_path = '/Users/weiran/YZT/project/stock_full_.xlsx'
    stock_full_file_path = '/Users/apple/Downloads/科科/MF703_FinalProject/data/stock_full_.xlsx'
    df = pd.read_csv(stock_full_file_path)
    df = df.pct_change()
    df = df.tail(-1)
    df = df + 1
    df = df.cumprod()
...
'''
deep nnf partial build which is the same as original deep nnf
...
'''
class DeepNNFPartial(nn.Module):
    def __init__(self, input_dim=300, hidden_size1=300, hidden_size2=300,
                    hidden_size3=300, hidden_size4=300, hidden_size5=300, num_classes=300, num_selection=30, dropout_p=0.2):
        super(DeepNNFPartial, self).__init__()
    ...
    def forward(self, x):
        ...
        # 获取最大的 self.num_selection 个权重及其索引
        top_weights, top_indices = torch.topk(out, self.num_selection)
        # 过滤 x 以获取与最大权重相对应的特征值
        selected_x = x[top_indices]
        # 归一化权重使得总和为 1
        normalized_top_weights = top_weights / top_weights.sum()
        # 计算累计变化
        cumulative_change = torch.sum(normalized_top_weights * selected_x)
        return cumulative_change, normalized_top_weights, top_indices


def train_deep_nnf_partial(x_train, y_train, num_epochs=20):
    start_time_deep_nnf = time.time()

    deep_NNF_partial = DeepNNFPartial()
    deep_NNF_partial_loss_fun = torch.nn.MSELoss(reduction='mean')
    deep_NNF_partial_optimizer = torch.optim.Adam(deep_NNF_partial.parameters(), lr=1e-6)

    for epoch in range(num_epochs):
        # 正向传播
        y_train_pred, weights_tensor, indices_tensor = deep_NNF_partial(x_train)
    ...
    # 转换权重为numpy数组并提取非零权重的股票
    final_weights = weights_tensor.detach().numpy()
    selected_stocks = indices_tensor.detach().numpy()

    return selected_stocks, final_weights


def train_deep_nnf(x_train, y_train, num_epochs=20):
    start_time_deep_nnf = time.time()
...
def partial(x_train, x_test, weights, stocks_index, num=30):
    df_partial = pd.DataFrame({'x_train': x_train, 'x_test': x_test,
    ...
    return x_train, x_test, out_index
...
def load_data():
    ...
    stock_full_data = pd.read_excel(stock_full_file_path)
    output_label_matrix_data = pd.read_excel(output_label_matrix_file_path)

    csi300_data['Date'] = pd.to_datetime(csi300_data['Date'])
    stock_full_data['Date'] = pd.to_datetime(stock_full_data['Date'])
    output_label_matrix_data['Date'] = pd.to_datetime(output_label_matrix_data['Date'])

    # 将'Date'列设置为索引
    csi300_data.set_index('Date', inplace=True)
    stock_full_data.set_index('Date', inplace=True)
    output_label_matrix_data.set_index('Date', inplace=True)

    stocks_returns = calculate_daily_returns(stock_full_data)
    csi300_returns = calculate_daily_returns(csi300_data)

    stocks_returns.replace([np.inf, -np.inf], 0, inplace=True)
...
def calculate_annualized_return(cumulative_returns, total_days):
    if total_days == 0:
        ...
        return None
    return (cumulative_returns.iloc[-1]) ** (252 / total_days) - 1
...
def main():
    ...
    nnf_partial_cumulative_returns = pd.Series(dtype=float)
    nnf_cumulative_returns = pd.Series(dtype=float)
    ...
    for training_returns, test_returns, training_csi300_returns, test_csi300_returns, test_start, test_end in train_test_data_selected():
        ...
        # nnf_partial Optimization Method
        training_stock_cumulative_returns = data_process(training_returns)
        training_csi300_cumulative_returns = data_process(training_csi300_returns)
        selected_stocks_index_nnf_partial, selected_stocks_weights_nnf_partial = train_deep_nnf_partial(training_stock_cumulative_returns, training_csi300_cumulative_returns)
        selected_stocks_codes_nnf_partial = test_returns.columns[selected_stocks_index_nnf_partial]
        results_nnf_partial = backtest(test_returns, test_csi300_returns, selected_stocks_weights_nnf_partial, selected_stocks_codes_nnf_partial)
        
        # nnf Optimization Method
        selected_stocks_index_nnf, selected_stocks_weights_nnf = train_deep_nnf_partial(training_stock_cumulative_returns, training_csi300_cumulative_returns)
        selected_stocks_codes_nnf = test_returns.columns[selected_stocks_index_nnf]
        results_nnf = backtest(test_returns, test_csi300_returns, selected_stocks_weights_nnf, selected_stocks_codes_nnf)
        ...
        filtered_test_returns_nnf_partial = test_returns.loc[base_date:, selected_stocks_codes_nnf_partial]
        adjusted_cumulative_return_nnf_partial = calculate_cumulative_return(filtered_test_returns_nnf_partial, selected_stocks_weights_nnf_partial)
        
        filtered_test_returns_nnf = test_returns.loc[base_date:, selected_stocks_codes_nnf]
        adjusted_cumulative_return_nnf = calculate_cumulative_return(filtered_test_returns_nnf, selected_stocks_weights_nnf)
        ...
        nnf_partial_results.append({
            "period": (test_start, test_end),
            "selected_stocks_and_weights": list(zip(selected_stocks_codes_nnf_partial, selected_stocks_weights_nnf_partial)),
            "results": results_nnf_partial,
            "adjusted_cumulative_return": adjusted_cumulative_return_nnf_partial,
            "method": "NNF Partial"
        })
        
        nnf_results.append({
            "period": (test_start, test_end),
            "selected_stocks_and_weights": list(zip(selected_stocks_codes_nnf, selected_stocks_weights_nnf)),
            "results": results_nnf,
            "adjusted_cumulative_return": adjusted_cumulative_return_nnf,
            "method": "NNF"
        })
        ...
        nnf_partial_cumulative_returns = pd.concat([nnf_partial_cumulative_returns, adjusted_cumulative_return_nnf_partial])
        nnf_cumulative_returns = pd.concat([nnf_cumulative_returns, adjusted_cumulative_return_nnf])
        
    all_results = []

    # Combine the results into one DataFrame
    for method_results in [ga_results, slsop_results, lasso_results, nnf_partial_results, nnf_results]:
        for result in method_results:
            result_df = pd.DataFrame([result['results']])
            result_df['period'] = str(result['period'])
            result_df['method'] = result['method']
            all_results.append(result_df)

    all_results_df = pd.concat(all_results, ignore_index=True)

    # Write the DataFrame to an Excel file
    all_results_df.to_excel('/Users/apple/Downloads/科科/MF703_FinalProject/data/results.xlsx', index=False)



    # Print the results for all methods
    for ga_result, slsop_result, lasso_result, nnf_partial_result, nnf_result in zip(ga_results, slsop_results, lasso_results, nnf_partial_results, nnf_results):
    ...
    plt.plot(nnf_partial_cumulative_returns, label="NNF Partial Cumulative Returns", linestyle='-')
    plt.plot(nnf_cumulative_returns, label="NNF Cumulative Returns", linestyle='-')
